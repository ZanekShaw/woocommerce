...
class TheClassServiceProvider extends AbstractServiceProvider {
	...
	public function register() {
		$this->add( ActionsProxy::class )->addArgument( TheDependencyClass::class );
	}
}
```
...
The callable will be executed and the result value will be returned when the registerd class name is resolved. Example:
...
Note that if the callable is defined as a function with arguments, the supplied parameters will be resolved too. 
